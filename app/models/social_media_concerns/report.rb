module SocialMediaConcerns
  class Report < ApplicationRecord
    ...
    def raise_already_followd_error_if_required
      raise SocialMediaConcerns::Reports::AlreadyExistsError.new("#{owner_type} has already reported that #{target_type}") if owner.reportable_objects.exists?(target: target)
    end
  end
end
